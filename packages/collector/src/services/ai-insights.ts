...
export function startInsightGenerator(intervalMs = 60_000): void {
  insightInterval = setInterval(async () => {
    try {
      if (!env.anthropicApiKey) return;

      const db = getPostgres();
      // Re-generate insights for active projects
      const projects = await db.query("SELECT id FROM projects");
      for (const { id } of projects.rows) {
        const ch = getClickHouse();
        const filesResult = await ch.query({
          query: `
            SELECT DISTINCT file FROM spans
            WHERE project_id = {projectId:String}
...
export function stopInsightGenerator(): void {
  if (insightInterval) {
