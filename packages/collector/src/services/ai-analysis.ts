...
/**
 * Runs a full analysis suite for all active projects.
 * Called every 30 minutes (anomalies + issues) or daily (digest, patterns).
 */
export function startAnalysisEngine(intervalMs = 30 * 60 * 1000): void {
  ...
  const runCycle = async () => {
    try {
      ...
      for (const { id } of projects.rows) {
        // Always run anomalies + issues (most time-sensitive)
        await runAnalysis({ projectId: id, type: "anomalies" }).catch(console.error);
        await runAnalysis({ projectId: id, type: "issues" }).catch(console.error);

        // Run patterns and digest less frequently (check if stale > 24h)
        const stale = await db.query(
          `SELECT type FROM ai_analyses
           WHERE project_id = $1 AND type IN ('patterns', 'weekly_digest')
             AND generated_at < now() - INTERVAL '24 hours'`,
          [id],
        );
        const staleTypes = new Set(stale.rows.map((r: any) => r.type));

        // Also run if no record exists
        const existing = await db.query(
          `SELECT type FROM ai_analyses WHERE project_id = $1 AND type IN ('patterns', 'weekly_digest')`,
          [id],
        );
        const existingTypes = new Set(existing.rows.map((r: any) => r.type));

        if (staleTypes.has("patterns") || !existingTypes.has("patterns")) {
          await runAnalysis({ projectId: id, type: "patterns" }).catch(console.error);
        }
        if (staleTypes.has("weekly_digest") || !existingTypes.has("weekly_digest")) {
          await runAnalysis({ projectId: id, type: "weekly_digest" }).catch(console.error);
        }
      }
    } catch (err) {
  ...
  // Run first cycle after a short delay (let data settle)
  setTimeout(() => runCycle(), 10_000);
  analysisInterval = setInterval(runCycle, intervalMs);
}

export function stopAnalysisEngine(): void {
  if (analysisInterval) {
